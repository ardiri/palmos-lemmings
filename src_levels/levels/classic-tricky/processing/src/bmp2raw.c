...
main(int argc, char *argv[])
{
  ...
  if (argc == 4) 
  {
    ...
    if (ok)
    {
      ...
      sprintf(strstr(argv[1], ".")+1, "raw");
      fout = fopen(argv[1], "wb");
      fwrite(rdata,1,c,fout);
      fclose(fout);
      printf("data: --> %s\n", argv[1]);
      ...
      sprintf(strstr(argv[2], ".")+1, "raw");
      fout = fopen(argv[2], "wb");
      fwrite(rmask,1,c,fout);
      fclose(fout);
      printf("mask: --> %s\n", argv[2]);
    }
  ...
  else 
  {
    ...
    fprintf(stderr, "  bmp2raw lev-data.bmp lev-mask.bmp lev.pal\n\n");
